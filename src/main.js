...
async function mergeMultipleSources({ urls, uploadedFiles, workDir, jobId, onProgress }) {
    ...
    await new Promise((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        if (code === 0 && fs.existsSync(outPath)) {
          ...
          reject(new Error(`URL ${i + 1} 다운로드 실패`));
        }
      });
  ...
  for (let i = 0; i < uploadedFiles.length; i++) {
    ...
    if (f.type === 'video') {
      ...
      onProgress('download', 30 + (i * 5), `이미지 ${i + 1} 변환 중...`);
      ...
      await new Promise((resolve, reject) => {
        ...
        proc.on('close', (code) => {
          if (code === 0 && fs.existsSync(imgVideoPath)) {
            ...
            reject(new Error(`이미지 ${i + 1} 변환 실패`));
          }
        });
        proc.on('error', reject);
      });
  ...
  // 3. 합치기
  onProgress('download', 50, '영상 합치는 중...');
...
ipcMain.handle('cancel-video', (event, jobId) => {
  ...
  if (jobId && activeJobs.has(jobId)) {
    ...
    console.log(`[cancel] ${jobId}: cancelled 플래그 설정, ${killed}개 프로세스 종료`);
    return true;
  }
  ...
  console.log(`[cancel] 전체 취소: ${activeJobs.size}개 작업, ${totalKilled}개 프로세스 종료`);
  return true;
});
...
// ===== 영상 처리 (기존) =====
ipcMain.handle('process-video', async (event, { url, config }) => {
  ...
  try {
    ...
    if (needsMerging) {
      ...
      const mergedPath = await mergeMultipleSources({
        urls, uploadedFiles, workDir, jobId, onProgress: sendProgress,
      });
...
ipcMain.handle('analyze-and-generate-script', async (event, params) => {
  const { videoUrl, productInfo, hookText, outroText, speechStyle, scriptLength, targetDurationSec } = params;
  ...
  try {
    ...
    if (!videoUrl || !videoUrl.trim()) {
      return { success: false, error: '영상 URL을 입력해주세요.' };
    }
    ...
    await new Promise((resolve, reject) => {
      ...
      proc.on('close', code => code === 0 ? resolve() : reject(new Error('오디오 추출 실패')));
      proc.on('error', reject);
    });
...
8. CTA (1~2자막): 부담 없이 ("필요하면 확인", "궁금하면 링크")

⚠️ **"꼭 사야 한다"식 강매 멘트 금지!** 
- ❌ "안 사면 후회", "빨리 사야 해", "지금 안 사면 손해"
- ✅ "필요한 사람한테 추천", "궁금하면 한번 보세요", "써보면 알아요"
    ...
    const callLLM = async (currentPrompt, retryCount = 0) => {
      try {
        ...
        if (error.response && error.response.status === 429 && retryCount < 2) {
          // retry-after 헤더 확인 (서버가 알려주는 대기 시간)
          const retryAfterHeader = error.response.headers['retry-after'];
          let waitSec = 30;  // 기본 30초 대기
          if (retryAfterHeader) {
            const parsed = parseFloat(retryAfterHeader);
            if (isFinite(parsed) && parsed > 0) waitSec = Math.min(60, Math.ceil(parsed));
          }
          console.log(`[script-gen] ⏳ Rate limit (429) 걸림. ${waitSec}초 후 자동 재시도... (시도 ${retryCount + 1}/2)`);
          sendProgress(`⏳ API 요청 많음, ${waitSec}초 대기 중...`);
          await new Promise(resolve => setTimeout(resolve, waitSec * 1000));
          return callLLM(currentPrompt, retryCount + 1);
        }
        
        // 🆕 429 에러인데 재시도도 실패한 경우 - 친절한 메시지
        if (error.response && error.response.status === 429) {
          throw new Error(
            '⏱️ API 요청 제한 초과!\n\n' +
            '너무 많은 요청을 짧은 시간에 보냈어요.\n' +
            '💡 해결 방법:\n' +
            '• 2~3분 기다린 후 다시 시도\n' +
            '• Groq 사용 중이면 OpenAI API 키 추가 등록 (설정에서)\n' +
            '• 유료 플랜 업그레이드 (Groq는 Console에서 확인 가능)'
          );
        }
